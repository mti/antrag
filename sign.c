...
void H(const uint8_t* m_r, poly* c1){
  ...
  ctr = 0;
  shake128_absorb(state, m_r, MSG_BYTES+ANTRAG_K/8);
  while(ctr < ANTRAG_D) 
  {
    shake128_squeezeblocks(buf,1,state);
    for(i=0;i<SHAKE128_RATE && ctr < ANTRAG_D;i+=2)
    {
      val = (buf[i] | ((uint16_t) buf[i+1] << 8));
      if(val < 5*ANTRAG_Q)
      {
        c1->coeffs[ctr].v = (double)(val%ANTRAG_Q);
        ctr++;
      }
    }
  }
...
void sign(const uint8_t* m, const secret_key* sk, signature* s){
  int trials = 0;
  poly c1, c2, v0, v1;
  uint8_t m_r[MSG_BYTES+ANTRAG_K/8];
  for(int i=0; i < MSG_BYTES; ++i) m_r[i] = m[i];

  do{
    trials++;
    randombytes(m_r+MSG_BYTES, ANTRAG_K/8);
    H(m_r, &c2);
    for(int i=0; i < ANTRAG_D; ++i) c1.coeffs[i].v = 0;
  ...
  set_poly(&(s->s1), &c1);
  set_poly(&(s->s2), &c2);
  for(int i=0; i < ANTRAG_K/8; ++i) (s->r)[i] = m_r[i+MSG_BYTES];
...
int verify(uint8_t* m, public_key* pk, signature* s){
  ...
  uint8_t m_r[MSG_BYTES+ANTRAG_K/8];
  for(int i=0; i < MSG_BYTES; ++i) m_r[i] = m[i];
  for(int i=0; i < ANTRAG_K/8; ++i) m_r[i+MSG_BYTES] = (s->r)[i];
  H(m_r, &c1);
